...
<a href="{{url('dashboard')}}"><button class="btn btn-primary mb-5">Back</button></a>
    <h1>{{$post->title}}</h1>
...
<div class="site-cover site-cover-sm same-height overlay single-page" style="background-image: url('{{ asset('images/img_2.jpg') }}');"
>
    <div class="container">
    <div class="row same-height justify-content-center">
    <div class="col-md-12 col-lg-10">
    <div class="post-entry text-center">
    <span class="post-category text-white bg-success mb-3">{{$post->Category->name ?? 'N/A'}}</span>
    <h1 class="mb-4"><a href="#">{{$post->title}}</a></h1>
    <div class="post-meta align-items-center text-center">
    <figure class="author-figure mb-0 mr-3 d-inline-block"><img src="{{ asset('images/person_1.jpg') }}" alt="Person 1" class="img-fluid"
        alt="Image" class="img-fluid"></figure>
    <span class="d-inline-block mt-1">{{$post->name}}</span>
    <span>&nbsp;-&nbsp; {{$post->created_at->format('d/m/Y')}}</span><br>
  
    <div id="app">
        <nav class="navbar navbar-expand-md col-9 ms-auto" style="background: transparent">
                ...
                <div class="collapse navbar-collapse ms-5" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav ms-5">
                        ...
                        @guest
                            @if (Route::has('login'))
                                <li class="nav-item">
                                    <a class="nav-link btn btn-primary bg-light me-4" href="{{ route('login') }}">{{ __('Login') }}</a>
                                </li>
                            @endif
                            @if (Route::has('register'))
                            <li class="nav-item">
                                <a class="nav-link btn btn-primary bg-light" href="{{ route('register') }}">{{ __('Register') }}</a>
                            </li>
                        @endif
                        
                        @else
                            <li class="nav-item dropdown">
                                <a id="navbarDropdown" class="nav-link dropdown-toggle btn btn-primary bg-light" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                    {{ Auth::user()->name }}
                                </a>
    ...
    <img src="/postimage/{{$post->image}}" alt="Image placeholder" class="img-fluid rounded">
    </div>
    <div class="col-md-6 mb-4">
    <img src="{{ asset('images/img_3.jpg') }}" alt="Image placeholder" class="img-fluid rounded">
    </div>
    <div class="col-md-6 mb-4">
    <img src="{{ asset('images/img_3.jpg') }}" alt="Image placeholder" class="img-fluid rounded">
    </div>
    ...
    <div>
        ...
        <p>{{ $comment->comment }}</p>
        {{-- <p>{{ $comment->id }}</p> --}}
        <a href="javascript:void(0);" onclick="reply(this)" class="text-decoration-none text-dark reply-link" data-comment-id="{{ $comment->id }}">Reply</a>
        
        <!-- Replies for this comment -->
        @foreach ($comment->reply as $replys)
        <div style="padding-left: 3%; padding-bottom: 10px;">
                {{-- <div style="padding-left: 6%; padding-bottom: 10px;"> --}}
                    <b>{{ $replys->name }}</b>
                    <p>{{ $replys->reply }}</p>
                    {{-- <p>{{ $replys->comment_id ??'N/A' }}</p> --}}
                    <a href="javascript:void(0);" onclick="reply(this)" class="text-decoration-none text-dark reply-link" data-comment-id="{{ $comment->id }}">Reply</a>
                {{-- </div> --}}
            </div>
            @endforeach
    ...
    <div class="replyDiv" style="display: none;">
        <form action="{{ url('add_reply') }}" method="POST">
            ...
            <input type="text" id="comment_id" value="{{$comment->id}}"   name="commant_id" >
            {{-- @endforeach --}}
            <textarea class="form-control" name="reply" placeholder="Reply Here" rows="4"></textarea><br>
            {{-- @foreach($reply as $replys) --}}
            {{-- <input type="" name="reply_id" value="{{ $replys->id }}">  --}}
            {{-- @endforeach --}}
    ...
    <div class="pt-5">
    <h3 class="mb-5">Add Comments</h3>
  
    <form action="{{url('add_comment')}}" method="POST">
        @csrf
        <textarea class="mb-4" name="comment" placeholder="Comment Here" id="" cols="100" rows="10"></textarea><br>
    ...
    <p><a href="{{url('profile/user',$post->id)}}" class="btn btn-primary btn-sm rounded px-4 py-2">Read my bio</a></p>
    <p class="social">
    ...
    <script type="text/javascript">
        function reply(caller){
            // document.getElementById('commant_id');
